...
def main():
    ...
    if args.callback_path:
        callbacks = TrainCallbacks(
            on_update_train_progress=lambda *fargs: write_request(
                args.callback_path, "on_update_train_progress", *fargs
            ),
            ...
            on_update_sample_default_progress=lambda *fargs: write_request(
                args.callback_path, "on_update_sample_default_progress", *fargs
            ),
            on_sample_custom=lambda *fargs: write_request(args.callback_path, "on_sample_custom", *fargs),
            on_update_sample_custom_progress=lambda *fargs: write_request(
                args.callback_path, "on_update_sample_custom_progress", *fargs
            ),
        )
    else:
        callbacks = TrainCallbacks()
    ...
    if args.command_path:
        ...
        command_thread = threading.Thread(
            target=command_thread_function, args=(commands, args.command_path, stop_event)
        )
