...
from modules.util.checkpointing_util import (
    enable_checkpointing_for_lens_transformer,
)
...
class BaseLensSetup(
    ...
    def setup_optimizations(
        ...
        #model.text_encoder is not quantized - GPT OSS is loaded in MXFP4
    ...
    def predict(
        ...
        with model.autocast_context:
            ...
            text_encoder_output, tokens_mask = model.encode_text(
            ...
            packed_latent_input = model.pack_latents(scaled_noisy_latent_image)

            packed_predicted_flow = model.transformer(
                hidden_states=packed_latent_input.to(dtype=model.train_dtype.torch_dtype()),
                encoder_hidden_states=[f.to(dtype=model.train_dtype.torch_dtype()) for f in text_encoder_output],
                encoder_hidden_states_mask=tokens_mask,
    ...
    def prepare_text_caching(self, model: LensModel, config: TrainConfig):
        model.release()
