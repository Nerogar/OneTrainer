...
class ModelSetupDiffusionLossMixin(metaclass=ABCMeta):
            ...
            losses += (
                masked_losses_with_prior(
                    losses=self.__log_cosh_loss(
                        data["predicted"].to(dtype=torch.float32), data["target"].to(dtype=torch.float32)
                    ),
                    prior_losses=self.__log_cosh_loss(
                        data["predicted"].to(dtype=torch.float32), data["prior_target"].to(dtype=torch.float32)
                    )
                    if "prior_target" in data
                    else None,
                    mask=batch["latent_mask"].to(dtype=torch.float32),
    ...
    def __unmasked_losses(
        ...
        if config.mse_strength != 0:
            losses += (
                F.mse_loss(
                    data["predicted"].to(dtype=torch.float32), data["target"].to(dtype=torch.float32), reduction="none"
                ).mean(mean_dim)
                * config.mse_strength
            )
