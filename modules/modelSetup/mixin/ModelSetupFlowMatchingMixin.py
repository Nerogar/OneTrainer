    ...
    def _add_noise_discrete(
        ...
        if self.__sigma is None:
            ...
            all_timesteps = torch.arange(
                start=1, end=num_timesteps + 1, step=1, dtype=torch.int32, device=scaled_latent_image.device
            )
        ...
        scaled_noisy_latent_image = (
            latent_noise.to(dtype=sigmas.dtype) * sigmas + scaled_latent_image.to(dtype=sigmas.dtype) * one_minus_sigmas
        )
