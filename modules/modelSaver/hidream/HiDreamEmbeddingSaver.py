    ...
    def _to_state_dict(
        ...
        if embedding is not None:
            ...
            if embedding.text_encoder_3_embedding.vector is not None:
                state_dict["t5"] = embedding.text_encoder_3_embedding.vector.to(device="cpu", dtype=dtype)
            if embedding.text_encoder_4_embedding.vector is not None:
                state_dict["llama"] = embedding.text_encoder_4_embedding.vector.to(device="cpu", dtype=dtype)
            if embedding.text_encoder_1_embedding.output_vector is not None:
                state_dict["clip_l_out"] = embedding.text_encoder_1_embedding.output_vector.to(
            ...
            if embedding.text_encoder_3_embedding.output_vector is not None:
                state_dict["t5_out"] = embedding.text_encoder_3_embedding.output_vector.to(device="cpu", dtype=dtype)
            if embedding.text_encoder_4_embedding.output_vector is not None:
                state_dict["llama_out"] = embedding.text_encoder_4_embedding.output_vector.to(device="cpu", dtype=dtype)
