...
def folder_scan(
    ...
    if advanced_checks:
        ...
        file_list_str = [
            x.path for x in file_list
        ]  # seems faster to check list of strings for matching files than list of path objects
    ...
    for path in file_list:
        ...
        if (
            extension.lower() in img_extensions_list
            ...
            if advanced_checks:
                    ...
                    with open(basename + ".txt", "r") as captionfile:
                        ...
                        for caption in captionlist:
                            ...
                            if char_count > stats_dict["max_caption_length"][0]:
                                stats_dict["max_caption_length"] = [
                                    char_count,
                                    os.path.relpath(path, conceptconfig.path),
                                    word_count,
                                ]
                            if char_count < stats_dict["min_caption_length"][0]:
                                stats_dict["min_caption_length"] = [
                                    char_count,
                                    os.path.relpath(path, conceptconfig.path),
                                    word_count,
                                ]
                            stats_dict["avg_caption_length"][0] += (
                                char_count - stats_dict["avg_caption_length"][0]
                            ) / (stats_dict["image_count"] + stats_dict["video_count"])
                            stats_dict["avg_caption_length"][1] += (
                                word_count - stats_dict["avg_caption_length"][1]
                            ) / (stats_dict["image_count"] + stats_dict["video_count"])
                ...
                nearest_aspect = min(
                    aspect_ratio_list, key=lambda x: abs(x - true_aspect)
                )  # try to match math used in aspect bucketing
                stats_dict["aspect_buckets"][nearest_aspect] += 1

                if pixels > stats_dict["max_pixels"][0]:
                    stats_dict["max_pixels"] = [
                        pixels,
                        os.path.relpath(path, conceptconfig.path),
                        f"{width}w x {height}h",
                    ]
                if pixels < stats_dict["min_pixels"][0]:
                    stats_dict["min_pixels"] = [
                        pixels,
                        os.path.relpath(path, conceptconfig.path),
                        f"{width}w x {height}h",
                    ]
                stats_dict["avg_pixels"] += (pixels - stats_dict["avg_pixels"]) / (
                    stats_dict["image_count"] + stats_dict["video_count"]
                )
...
def combine_stats_dicts(input_dicts: list[dict], advanced_checks: bool):
    ...
    for dict in input_dicts:
        ...
        for key in dict:
            if (
                key in ["file_size", "image_count", "video_count", "mask_count", "caption_count", "directory_count"]
                or advanced_checks
                and key
                in [
                    "image_with_mask_count",
        ...
        for dir in subfolders:
            stats_futures += [
                executor.submit(
                    folder_scan,
                    dir,
                    init_concept_stats(advanced_checks),
                    advanced_checks,
                    start_time,
                    wait_time,
                    cancel_scan_flag,
            ...
            subfolders.extend(
                [f.path for f in os.scandir(dir) if f.is_dir() and not os.path.basename(f.path).startswith(".")]
            )
