...
def reduce_grads_mean(
    ...
    precision: GradientReducePrecision,
    after_reduce=None,
    async_op: bool = False,
    max_buffer: int = 0,
):
...
def complete_previous_async_ops(precision: GradientReducePrecision, next_size: int = 0, max_buffer: int = 0):
    global async_deque
    global in_transfer
    while async_deque and (in_transfer + next_size > max_buffer or async_deque[0][0].is_completed()):
...
def parameter_divergence(params: list[torch.Tensor], train_device: torch.device):
    ...
    for param in params:
        param_list = (
            [torch.zeros_like(param, device=train_device) for _ in range(world_size())] if is_master() else None
        )
