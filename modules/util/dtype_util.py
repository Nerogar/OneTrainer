...
def create_autocast_context(
    ...
    if len(weight_dtypes) == 1 and train_dtype == weight_dtypes[0]:
        ...
        return torch.autocast(
            device_type=device.type, dtype=train_dtype.torch_dtype(), cache_enabled=enable_autocast_cache
        ), train_dtype
...
def disable_fp16_autocast_context(
    ...
    return torch.autocast(
        device_type=device.type, dtype=fallback_train_dtype.torch_dtype(), cache_enabled=enable_autocast_cache
...
def disable_bf16_on_fp16_autocast_context(
    ...
    return torch.autocast(
        device_type=device.type, dtype=weight_dtypes[0], cache_enabled=enable_autocast_cache
    ), weight_dtypes[0]
