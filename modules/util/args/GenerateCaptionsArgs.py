...
class GenerateCaptionsArgs(BaseArgs):
    ...
    def parse_args() -> "GenerateCaptionsArgs":
        ...
        parser.add_argument(
            "--model",
            type=GenerateCaptionsModel,
            required=True,
            dest="model",
            help="The model to use when generating captions",
            choices=list(GenerateCaptionsModel),
        )
        parser.add_argument(
            "--sample-dir", type=str, required=True, dest="sample_dir", help="Directory where samples are located"
        )
        parser.add_argument(
            "--initial-caption",
            type=str,
            default="",
            required=False,
            dest="initial_caption",
            help="An initial caption to start generating from",
        )
        parser.add_argument(
            "--caption-prefix",
            type=str,
            ...
            required=False,
            dest="caption_prefix",
            help="Add this to the start of the generated caption (before initial caption)",
        )
        parser.add_argument(
            "--caption-postfix",
            type=str,
            default="",
            required=False,
            dest="caption_postfix",
            help="Add this to the end of the generated caption",
        )
        parser.add_argument(
            "--mode", type=str, default="fill", required=False, dest="mode", help="Either replace, fill or add"
        )
        parser.add_argument(
            "--device",
            type=str,
            required=False,
            default=default_device.type,
            dest="device",
            help="The device to use for calculations",
        )
        parser.add_argument(
            "--dtype",
            type=DataType,
            required=False,
            default=DataType.FLOAT_16,
            dest="dtype",
            help="The data type to use for weights during calculations",
