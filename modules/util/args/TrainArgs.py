...
class TrainArgs(BaseArgs):
    ...
    def parse_args() -> 'TrainArgs':
        ...
        # optimizer settings
        parser.add_argument("--optimizer_adam_w_mode", type=bool, default=True, dest="optimizer_adam_w_mode", help='Whether to use weight decay correction for Adam optimizer.')
        parser.add_argument("--optimizer_alpha", type=float, default=0.99, dest="optimizer_alpha", help='Smoothing parameter for RMSprop and others.')
        parser.add_argument("--optimizer_amsgrad", type=bool, default=False, dest="optimizer_amsgrad", help='Whether to use the AMSGrad variant for Adam.')
        parser.add_argument("--optimizer_beta1", type=float, default=None, dest="optimizer_beta1", help='optimizer_momentum term.')
        parser.add_argument("--optimizer_beta2", type=float, default=0.999, dest="optimizer_beta2", help='Coefficients for computing running averages of gradient.')
        parser.add_argument("--optimizer_beta3", type=float, default=None, dest="optimizer_beta3", help='Coefficient for computing the Prodigy stepsize.')
        parser.add_argument("--optimizer_bias_correction", type=bool, default=True, dest="optimizer_bias_correction", help='Whether to use bias correction in optimization algorithms like Adam.')
        parser.add_argument("--optimizer_block_wise", type=bool, default=True, dest="optimizer_block_wise", help='Whether to perform block-wise model update.')
        parser.add_argument("--optimizer_capturable", type=bool, default=False, dest="optimizer_capturable", help='Whether some property of the optimizer can be captured.')
        parser.add_argument("--optimizer_centered", type=bool, default=False, dest="optimizer_centered", help='Whether to center the gradient before scaling. Great for stabilizing the training process.')
        parser.add_argument("--optimizer_clip_threshold", type=float, default=1.0, dest="optimizer_clip_threshold", help='Clipping value for gradients.')
        parser.add_argument("--optimizer_d0", type=float, default=1e-6, dest="optimizer_d0", help='Initial D estimate for D-adaptation.')
        parser.add_argument("--optimizer_d_coef", type=float, default=1.0, dest="optimizer_d_coef", help='Coefficient in the expression for the estimate of d.')
        parser.add_argument("--optimizer_dampening", type=float, default=0.0, dest="optimizer_dampening", help='Dampening for optimizer_momentum.')
        parser.add_argument("--optimizer_decay_rate", type=float, default=-0.8, dest="optimizer_decay_rate", help='Rate of decay for moment estimation.')
        parser.add_argument("--optimizer_decouple", type=bool, default=False, dest="optimizer_decouple", help='Use AdamW style optimizer_decoupled weight decay.')
        parser.add_argument("--optimizer_differentiable", type=bool, default=True, dest="optimizer_differentiable", help='Whether the optimization function is optimizer_differentiable.')
        parser.add_argument("--optimizer_eps", type=float, default=1e-8, dest="optimizer_eps", help='A small value to prevent division by zero.')
        parser.add_argument("--optimizer_eps2", type=float, default=1e-3, dest="optimizer_eps2", help='A small value to prevent division by zero.')
        parser.add_argument("--optimizer_foreach", type=bool, default=False, dest="optimizer_foreach", help='If true, apply the optimizer to each parameter independently.')
        parser.add_argument("--optimizer_fsdp_in_use", type=bool, default=False, dest="optimizer_fsdp_in_use", help='Flag for using sharded parameters.')
        parser.add_argument("--optimizer_fused", type=bool, default=True, dest="optimizer_fused", help='Whether to use a optimizer_fused implementation if available.')
        parser.add_argument("--optimizer_growth_rate", type=float, default=float('inf'), dest="optimizer_growth_rate", help='Limit for D estimate growth rate.')
        parser.add_argument("--optimizer_initial_accumulator_value", type=int, default=0, dest="optimizer_initial_accumulator_value", help='Initial value for Adagrad optimizer.')
        parser.add_argument("--optimizer_is_paged", type=bool, default=False, dest="optimizer_is_paged", help='Whether the optimizer\'s internal state should be paged to CPU.')
        parser.add_argument("--optimizer_log_every", type=int, default=100, dest="optimizer_log_every", help='Intervals at which logging should occur.')
        parser.add_argument("--optimizer_lr_decay", type=int, default=0.0, dest="optimizer_lr_decay", help='Rate at which learning rate decreases.')
        parser.add_argument("--optimizer_max_unorm", type=float, default=0.02, dest="optimizer_max_unorm", help='Maximum value for gradient clipping by norms.')
        parser.add_argument("--optimizer_maximize", type=bool, default=False, dest="optimizer_maximize", help='Whether to optimizer_maximize the optimization function.')
        parser.add_argument("--optimizer_min_8bit_size", type=int, default=4096, dest="optimizer_min_8bit_size", help='Minimum tensor size for 8-bit quantization.')
        parser.add_argument("--optimizer_momentum", type=float, default=0.99, dest="optimizer_momentum", help='Factor to accelerate SGD in relevant direction.')
        parser.add_argument("--optimizer_nesterov", type=bool, default=False, dest="optimizer_nesterov", help='Whether to enable Nesterov optimizer_momentum.')
        parser.add_argument("--optimizer_no_prox", type=bool, default=False, dest="optimizer_no_prox", help='Whether to use proximity updates or not.')
        parser.add_argument("--optimizer_optim_bits", type=int, default=32, dest="optimizer_optim_bits", help='Number of bits used for optimization.')
        parser.add_argument("--optimizer_percentile_clipping", type=int, default=100, dest="optimizer_percentile_clipping", help='Gradient clipping based on percentile values.')
        parser.add_argument("--optimizer_relative_step", type=bool, default=True, dest="optimizer_relative_step", help='Whether to use a relative step size.')
        parser.add_argument("--optimizer_safeguard_warmup", type=bool, default=True, dest="optimizer_safeguard_warmup", help='Avoid issues during warm-up stage.')
        parser.add_argument("--optimizer_scale_parameter", type=bool, default=True, dest="optimizer_scale_parameter", help='Whether to scale the parameter or not.')
        parser.add_argument("--optimizer_use_bias_correction", type=bool, default=True, dest="optimizer_use_bias_correction", help='Turn on Adam\'s bias correction.')
        parser.add_argument("--optimizer_use_triton", type=bool, default=False, dest="optimizer_use_triton", help='Whether Triton optimization should be used.')
        parser.add_argument("--optimizer_warmup_init", type=bool, default=False, dest="optimizer_warmup_init", help='Whether to warm-up the optimizer initialization.')
        parser.add_argument("--optimizer_weight_decay", type=float, default=1e-2, dest="optimizer_weight_decay", help='Regularization to prevent overfitting.')
