...
class TrainCallbacks:
    def __init__(
            self,
            on_update_train_progress: Callable[[TrainProgress, int, int, float, float], None] = lambda _, __, ___, ____, _____: None,
    ...
    def on_update_train_progress(self, train_progress: TrainProgress, max_step: int, max_epoch: int, step_rate: float | None, epoch_rate: float | None):
        if self.__on_update_train_progress:
            with contextlib.suppress(Exception):
                self.__on_update_train_progress(train_progress, max_step, max_epoch, step_rate, epoch_rate)
