...
def __map_transformer_attention_block(in_states: dict, out_prefix: str, in_prefix: str) -> dict:
    ...
    out_states |= util.map_wb(
        in_states, util.combine(out_prefix, "attn.proj"), util.combine(in_prefix, "attn1.to_out.0")
    )

    out_states |= util.map_wb(
        in_states, util.combine(out_prefix, "cross_attn.q_linear"), util.combine(in_prefix, "attn2.to_q")
    )
    ...
    out_states |= util.map_wb(
        in_states, util.combine(out_prefix, "cross_attn.proj"), util.combine(in_prefix, "attn2.to_out.0")
    )
...
def __map_transformer(in_states: dict, out_prefix: str, in_prefix: str, model_type: ModelType) -> dict:
    ...
    if model_type.is_pixart_alpha():
        out_states |= util.map_wb(
            in_states,
            util.combine(out_prefix, "ar_embedder.mlp.0"),
            util.combine(in_prefix, "adaln_single.emb.aspect_ratio_embedder.linear_1"),
        )
        out_states |= util.map_wb(
            in_states,
            util.combine(out_prefix, "ar_embedder.mlp.2"),
            util.combine(in_prefix, "adaln_single.emb.aspect_ratio_embedder.linear_2"),
        )

        out_states |= util.map_wb(
            in_states,
            util.combine(out_prefix, "csize_embedder.mlp.0"),
            util.combine(in_prefix, "adaln_single.emb.resolution_embedder.linear_1"),
        )
        out_states |= util.map_wb(
            in_states,
            util.combine(out_prefix, "csize_embedder.mlp.2"),
            util.combine(in_prefix, "adaln_single.emb.resolution_embedder.linear_2"),
        )

    # out_states[util.combine(out_prefix, "y_embedder.y_embedding")] = __generate_y_embedding() #TODO
    out_states |= util.map_wb(
        in_states,
        util.combine(out_prefix, "y_embedder.y_proj.fc1"),
        util.combine(in_prefix, "caption_projection.linear_1"),
    )
    out_states |= util.map_wb(
        in_states,
        util.combine(out_prefix, "y_embedder.y_proj.fc2"),
        util.combine(in_prefix, "caption_projection.linear_2"),
    )
    ...
    out_states |= util.map_wb(
        ...
        util.combine(in_prefix, "adaln_single.emb.timestep_embedder.linear_1"),
    )
    out_states |= util.map_wb(
        in_states,
        util.combine(out_prefix, "t_embedder.mlp.2"),
        util.combine(in_prefix, "adaln_single.emb.timestep_embedder.linear_2"),
    )

    out_states |= util.map_wb(
        in_states, util.combine(out_prefix, "t_block.1"), util.combine(in_prefix, "adaln_single.linear")
    )

    out_states["pos_embed"] = __generate_pos_embed()

    for i in range(28):
        out_states |= __map_transformer_attention_block(
            in_states, util.combine(out_prefix, f"blocks.{i}"), util.combine(in_prefix, f"transformer_blocks.{i}")
        )

    out_states |= util.map_wb(
        in_states, util.combine(out_prefix, "final_layer.linear"), util.combine(in_prefix, "proj_out")
    )
    out_states[util.combine(out_prefix, "final_layer.scale_shift_table")] = in_states[
        util.combine(in_prefix, "scale_shift_table")
    ]
