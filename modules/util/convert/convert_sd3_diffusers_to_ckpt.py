...
def __map_transformer_block(in_states: dict, out_prefix: str, in_prefix: str, is_last: bool) -> dict:
    ...
    out_states |= util.map_wb(
        in_states, util.combine(out_prefix, "x_block.attn.proj"), util.combine(in_prefix, "attn.to_out.0")
    )

    if not is_last:
        out_states |= util.map_wb(
            in_states, util.combine(out_prefix, "context_block.attn.proj"), util.combine(in_prefix, "attn.to_add_out")
        )

    out_states |= util.map_wb(
        in_states, util.combine(out_prefix, "x_block.adaLN_modulation.1"), util.combine(in_prefix, "norm1.linear")
    )

    if not is_last:
        out_states |= util.map_wb(
            in_states,
            util.combine(out_prefix, "context_block.adaLN_modulation.1"),
            util.combine(in_prefix, "norm1_context.linear"),
        )
    else:
        out_states[util.combine(out_prefix, "context_block.adaLN_modulation.1.weight")] = __swap_chunks(
            in_states[util.combine(in_prefix, "norm1_context.linear.weight")]
        )
        out_states[util.combine(out_prefix, "context_block.adaLN_modulation.1.bias")] = __swap_chunks(
            in_states[util.combine(in_prefix, "norm1_context.linear.bias")]
        )

    if util.combine(in_prefix, "attn.norm_added_k.weight") in in_states:
        out_states[util.combine(out_prefix, "context_block.attn.ln_k.weight")] = in_states[
            util.combine(in_prefix, "attn.norm_added_k.weight")
        ]
        out_states[util.combine(out_prefix, "context_block.attn.ln_q.weight")] = in_states[
            util.combine(in_prefix, "attn.norm_added_q.weight")
        ]

    out_states |= util.map_wb(
        in_states, util.combine(out_prefix, "x_block.mlp.fc1"), util.combine(in_prefix, "ff.net.0.proj")
    )
    out_states |= util.map_wb(
        in_states, util.combine(out_prefix, "x_block.mlp.fc2"), util.combine(in_prefix, "ff.net.2")
    )

    if util.combine(in_prefix, "attn.norm_k.weight") in in_states:
        out_states[util.combine(out_prefix, "x_block.attn.ln_k.weight")] = in_states[
            util.combine(in_prefix, "attn.norm_k.weight")
        ]
        out_states[util.combine(out_prefix, "x_block.attn.ln_q.weight")] = in_states[
            util.combine(in_prefix, "attn.norm_q.weight")
        ]

    if util.combine(in_prefix, "attn2.norm_k.weight") in in_states:
        out_states[util.combine(out_prefix, "x_block.attn2.ln_k.weight")] = in_states[
            util.combine(in_prefix, "attn2.norm_k.weight")
        ]
        out_states[util.combine(out_prefix, "x_block.attn2.ln_q.weight")] = in_states[
            util.combine(in_prefix, "attn2.norm_q.weight")
        ]
        ...
        out_states |= util.map_wb(
            in_states, util.combine(out_prefix, "x_block.attn2.proj"), util.combine(in_prefix, "attn2.to_out.0")
        )

    if not is_last:
        out_states |= util.map_wb(
            in_states,
            util.combine(out_prefix, "context_block.mlp.fc1"),
            util.combine(in_prefix, "ff_context.net.0.proj"),
        )
        out_states |= util.map_wb(
            in_states, util.combine(out_prefix, "context_block.mlp.fc2"), util.combine(in_prefix, "ff_context.net.2")
        )
...
def __map_transformer(in_states: dict, out_prefix: str, in_prefix: str) -> dict:
    ...
    out_states |= util.map_wb(
        in_states, util.combine(out_prefix, "x_embedder.proj"), util.combine(in_prefix, "pos_embed.proj")
    )
    out_states |= util.map_wb(
        in_states, util.combine(out_prefix, "context_embedder"), util.combine(in_prefix, "context_embedder")
    )
    ...
    while any(key.startswith(util.combine(in_prefix, f"transformer_blocks.{i}")) for key in in_states):
        ...
        out_states |= __map_transformer_block(
            in_states,
            util.combine(out_prefix, f"joint_blocks.{i}"),
...
def __map_t5_text_encoder(in_states: dict, out_prefix: str, in_prefix: str) -> dict:
    ...
    out_states[util.combine(out_prefix, "encoder.embed_tokens.weight")] = in_states[
        util.combine(in_prefix, "encoder.embed_tokens.weight")
    ].clone()
