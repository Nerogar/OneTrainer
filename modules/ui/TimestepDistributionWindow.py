...
class TimestepGenerator(ModelSetupNoiseMixin):
    ...
    def generate(self) -> Tensor:
        ...
        if self.timestep_distribution == TimestepDistribution.SPEED:
            # Create a linear beta schedule for visualization, as used in DDPM.
            ...
            # Calculate the asymmetric sampling probabilities, which is the core of SpeeD's sampling.
            grad = torch.gradient(sqrt_one_minus_alphas_cumprod)[0]
            threshold = 1e-4

            # We now calculate the probability distribution 'p' on the full schedule.
            p = torch.tanh(1e6 * (grad - threshold)) + 1.5

            # Normalize the full probabilities to ensure they sum to 1.
            p = torch.nn.functional.normalize(p, p=1, dim=0)

            # Perform the initial asymmetric sampling from the full distribution.
            # We sample half the batch size, as the other half will be generated by mirroring.
            ...
            # Perform the symmetric mirroring as described in the reference implementation.
            meaningful_steps_end = (grad < threshold).nonzero(as_tuple=True)[0][0].item()

            mirrored_samples = torch.where(
                initial_samples < meaningful_steps_end,
                meaningful_steps_end - initial_samples,
                initial_samples - meaningful_steps_end
            )

            # The final set of timesteps is the combination of the initial asymmetric samples and their mirrored counterparts.
            final_samples = torch.cat([initial_samples, mirrored_samples], dim=0)[:batch_size]

            # Apply the min/max noising strength after sampling and mirroring are complete.
            min_timestep = int(num_train_timesteps * self.min_noising_strength)
            max_timestep = int(num_train_timesteps * self.max_noising_strength)

            # Clamp the final values to the valid range.
            final_samples = torch.clamp(final_samples, min=min_timestep, max=max_timestep)
