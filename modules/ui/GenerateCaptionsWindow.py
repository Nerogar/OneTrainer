...
class GenerateCaptionsWindow(ctk.CTkToplevel):
    def __init__(self, parent, path, parent_include_subdirectories, *args, **kwargs):
        ...
        self.mode_var = ctk.StringVar(self, "创建如果不存在")
        self.modes = ["替换所有标题", "创建如果不存在", "添加为新行"]
        ...
        self.path_button = ctk.CTkButton(self.frame, width=30, text="更多", command=lambda: self.browse_for_path(self.path_entry))
        self.path_button.grid(row=1, column=1, sticky="e", padx=5, pady=5)

        self.caption_label = ctk.CTkLabel(self.frame, text="初始标题", width=100)
        ...
        self.prefix_label = ctk.CTkLabel(self.frame, text="标题前缀", width=100)
        ...
        self.postfix_label = ctk.CTkLabel(self.frame, text="标题后缀", width=100)
        ...
        self.mode_label = ctk.CTkLabel(self.frame, text="模式", width=100)
        ...
        self.include_subdirectories_label = ctk.CTkLabel(self.frame, text="包含子文件夹", width=100)
        ...
        self.progress_label = ctk.CTkLabel(self.frame, text="进度：0/0", width=100)
        ...
        self.create_captions_button = ctk.CTkButton(self.frame, text="创建标题", width=310, command=self.create_captions)
