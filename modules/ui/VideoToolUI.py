...
class VideoToolUI(ctk.CTkToplevel):
    ...
    def status_bar(self, master):
        ...
        self.preview_image_label = ctk.CTkLabel(
            master=frame, text="Preview image", image=self.preview_image, height=150, width=150, compound="top"
        )
    ...
    def __clip_extract_tab(self, master):
        ...
        # single video
        components.label(frame, 0, 0, "Single Video", tooltip="Link to single video file to process.")
        self.clip_single_entry = ctk.CTkEntry(frame, width=190)
        self.clip_single_entry.grid(row=0, column=1, sticky="w", padx=5, pady=5)
        self.clip_single_button = ctk.CTkButton(
            frame,
            width=30,
            text="...",
            command=lambda: self.__browse_for_file(
                self.clip_single_entry, [("Video files", " ".join(f"*{e}" for e in SUPPORTED_VIDEO_EXTENSIONS))]
            ),
        )
        ...
        self.clip_list_button = ctk.CTkButton(
            frame, width=30, text="...", command=lambda: self.__browse_for_dir(self.clip_list_entry)
        )
        self.clip_list_button.grid(row=2, column=1, sticky="e", padx=5, pady=5)
        components.button(frame, 2, 2, "Extract Directory", command=lambda: self.__extract_clips_button(True))

        # output directory
        components.label(frame, 3, 0, "Output", tooltip="Path to folder where extracted clips will be saved.")
        self.clip_output_entry = ctk.CTkEntry(frame, width=190)
        self.clip_output_entry.grid(row=3, column=1, sticky="w", padx=5, pady=5)
        self.clip_output_button = ctk.CTkButton(
            frame, width=30, text="...", command=lambda: self.__browse_for_dir(self.clip_output_entry)
        )
        ...
        # Set FPS
        components.label(
            frame, 7, 0, "Set FPS", tooltip="FPS to convert output videos to, set to 0 to keep original rate."
        )
        ...
        components.label(frame, 8, 0, "Remove Borders", tooltip="Remove black borders from output clip")
    ...
    def __image_extract_tab(self, master):
        ...
        self.image_single_button = ctk.CTkButton(
            frame,
            width=30,
            text="...",
            command=lambda: self.__browse_for_file(
                self.image_single_entry, [("Video files", " ".join(f"*{e}" for e in SUPPORTED_VIDEO_EXTENSIONS))]
            ),
        )
        ...
        self.image_list_button = ctk.CTkButton(
            frame, width=30, text="...", command=lambda: self.__browse_for_dir(self.image_list_entry)
        )
        self.image_list_button.grid(row=2, column=1, sticky="e", padx=5, pady=5)
        components.button(frame, 2, 2, "Extract Directory", command=lambda: self.__extract_images_button(True))

        # output directory
        components.label(frame, 3, 0, "Output", tooltip="Path to folder where extracted images will be saved.")
        self.image_output_entry = ctk.CTkEntry(frame, width=190)
        self.image_output_entry.grid(row=3, column=1, sticky="w", padx=5, pady=5)
        self.image_output_button = ctk.CTkButton(
            frame, width=30, text="...", command=lambda: self.__browse_for_dir(self.image_output_entry)
        )
        ...
        components.label(frame, 7, 0, "Remove Borders", tooltip="Remove black borders from output image")
    ...
    def __video_download_tab(self, master):
        ...
        # link list
        components.label(frame, 1, 0, "Link List", tooltip="Path to txt file with list of links separated by newlines.")
        self.download_list_entry = ctk.CTkEntry(frame, width=190)
        self.download_list_entry.grid(row=1, column=1, sticky="w", padx=5, pady=5)
        self.download_list_button = ctk.CTkButton(
            frame,
            width=30,
            text="...",
        ...
        # output directory
        components.label(frame, 2, 0, "Output", tooltip="Path to folder where downloaded videos will be saved.")
        ...
        self.download_output_button = ctk.CTkButton(
            frame, width=30, text="...", command=lambda: self.__browse_for_dir(self.download_output_entry)
        )
        ...
        components.button(
            ...
            command=lambda: webbrowser.open(
                "https://github.com/yt-dlp/yt-dlp?tab=readme-ov-file#usage-and-options", new=0, autoraise=False
            ),
        )
    ...
    def __get_safe_fps(self, video: cv2.VideoCapture, video_path: str) -> float:
        ...
        if fps <= 0:
            self.__update_status(
                f'Warning: Could not read FPS for "{os.path.basename(video_path)}". Falling back to 30 FPS.'
            )
            return 30.0
        return fps

    @staticmethod
    def __get_output_dir(use_subdir: bool, batch_mode: bool, output_entry: str, video_path, input_dir: str) -> str:
        if use_subdir and batch_mode:
            return os.path.join(output_entry, os.path.splitext(os.path.relpath(video_path, input_dir))[0])
        elif use_subdir:
            return os.path.join(output_entry, os.path.splitext(os.path.basename(video_path))[0])
        return output_entry
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            for video_path in input_videos:
                output_directory = self.__get_output_dir(
                    self.output_subdir_clip_entry.get(), batch_mode, output_entry, video_path, input_multiple_entry
                )
                ...
                executor.submit(
                    self.__extract_clips,
                    str(video_path),
                    time_start,
                    time_end,
                    max_length,
                    self.split_at_cuts.get(),
                    bool(self.clip_bordercrop_entry.get()),
                    crop_variation,
                    target_fps,
                    output_directory,
                )
    ...
    def __extract_clips(
        ...
        for scene in scene_list:
            ...
            if length > max_length_frames:  # check for any scenes longer than max length
                ...
                new_splits = range(
                    scene[0], scene[1] + min_length_frames, new_length
                )  # divide clip into closest chunks to max_length
        ...
        self.__update_status(
            f'Video "{os.path.basename(video_path)}" being split into {len(scene_list_split)} clips in "{output_dir}"'
        )

        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            futures = [
                executor.submit(
                    self.__save_clip, scene, video_path, target_fps, remove_borders, crop_variation, output_dir
                )
    ...
    def __save_clip(
        ...
        if not success or frame is None:
            self.__update_status(
                f'Failed to read frame from "{os.path.basename(video_path)}" at {int(frame_number)}. Skipping clip.'
            )
            video.release()
            return
        ...
        self.__write_clip_av(video_path, output_path, scene, fps, target_fps, x1 + x2, y1 + y2, w2, h2)
    ...
    def __extract_images_multi(self, batch_mode: bool):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            for video_path in input_videos:
                output_directory = self.__get_output_dir(
                    self.output_subdir_img_entry.get(), batch_mode, output_entry, video_path, input_multiple_entry
                )
    ...
    def __save_frames(
        ...
        self.__update_status(
            f'Video "{os.path.basename(video_path)}" will be split into {len(frame_list)} images in "{output_dir}"'
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            for url in ydl_urls:
                executor.submit(
                    ...
                    url.strip(),
                    self.download_output_entry.get(),
