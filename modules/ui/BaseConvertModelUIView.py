...
class BaseConvertModelUIView:
    ...
    def build_content(self, frame, controller, ui_state, on_model_or_method_change):
        # model type
        self.components.label(frame, 0, 0, "模型类型",
                         tooltip="模型类型")
        ...
        # training method
        self.components.label(frame, 1, 0, "模型类型",
                         tooltip="要转换的模型类型")
        self.components.options_kv(frame, 1, 1, [
            ("基础模型", TrainingMethod.FINE_TUNE),
            ("LoRA", TrainingMethod.LORA),
            ("嵌入", TrainingMethod.EMBEDDING),
        ...
        # output data type
        self.components.label(frame, 3, 0, "输出数据类型",
        ...
        self.components.label(frame, 5, 0, "模型输出目标",
        ...
        self.button = self.components.button(frame, 6, 1, "转换", controller.convert_model)
    ...
    def build_dynamic_content(self, frame, controller, ui_state):
        ...
        if controller.convert_model_args.training_method in [TrainingMethod.LORA, TrainingMethod.EMBEDDING]:
            self.components.label(frame, row, 0, "基础模型名称",
                             tooltip="此LoRA/嵌入训练的基础模型")
            self.components.path_entry(
