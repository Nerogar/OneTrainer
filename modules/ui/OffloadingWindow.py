...
class OffloadingWindow(ctk.CTkToplevel):
    ...
    def __content_frame(self, master):
        ...
        components.options(
            frame, 0, 1, [str(x) for x in list(GradientCheckpointingMethod)], self.ui_state, "gradient_checkpointing"
        )

        # gradient checkpointing layer offloading
        components.label(frame, 1, 0, "Async Offloading", tooltip="Enables Asynchronous offloading.")
        components.switch(frame, 1, 1, self.ui_state, "enable_async_offloading")

        # gradient checkpointing layer offloading
        components.label(frame, 2, 0, "Offload Activations", tooltip="Enables Activation Offloading")
