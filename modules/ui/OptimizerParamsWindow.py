...
class OptimizerParamsWindow(ctk.CTkToplevel):
    ...
    def create_dynamic_ui(
        ...
        KEY_DETAIL_MAP = {
            ...
            'cautious': {'title': 'Cautious', 'tooltip': 'Outlined in Cautious Optimizers: Improving Training with One Line of Code. Applies a simple modification to parameter updates that promotes values that are aligned with the current gradient. This should result in faster convergence. While not 1:1 compatible with Schedule-Free, the implementation by nhamanasu does work, though improvements may be limited.', 'type': 'bool'},
            'weight_decay_by_lr': {'title': 'weight_decay_by_lr', 'tooltip': 'By default, weight decay is multiplied by the adaptive learning rate (as per the PyTorch implementation of AdamW). Disabling this feature will stop decay being multiplied by the LR. Please note this setting is completely different to Prodigy's "decouple" setting; this optimiser uses decoupled weight decay by default (equal to "decouple=True" for the reference implementation). Do not change this setting unless you know what you're doing!', 'type': 'bool'},
            'prodigy_steps': {'title': 'prodigy_steps', 'tooltip': 'Turn off Prodigy after N steps', 'type': 'int'},
            'use_speed': {'title': 'use_speed', 'tooltip': 'The authors creation that is dubbed Simplified Prodigy with rElativE D. It replaces Prodigys numerator/denominator ratio with a momentum-based estimate of directional progress. SPEED uses less memory, is scale-insensitive, and can be a better choice when training multiple networks, however, it can be unstable when used with weight decay or for extremely long training runs (where it is recommended to use prodigy_steps).', 'type': 'bool'},
            ...
            'factored': {'title': 'factored', 'tooltip': 'uses low-rank approximations for the second moment, much like Adafactor', 'type': 'bool'},
            'factored_fp32': {'title': 'factored_fp32', 'tooltip': 'uses low-rank approximations for the second moment, much like Adafactor, set this value to true to use fp32. Set this value to false to use bf16 to save extra memory', 'type': 'bool'},
            'use_stableadamw': {'title': 'use_stableadamw', 'tooltip': 'Use use_stableadamw for gradient scaling, which also reduces the need for external gradient scaling or clipping', 'type': 'bool'},
            'use_cautious': {'title': 'use_cautious', 'tooltip': 'Outlined in Cautious Optimizers: Improving Training with One Line of Code. Applies a simple modification to parameter updates that promotes values that are aligned with the current gradient. This should result in faster convergence. While not 1:1 compatible with Schedule-Free, the implementation by nhamanasu does work, though improvements may be limited.', 'type': 'bool'},
            'use_grams': {'title': 'use_grams', 'tooltip': 'Described in Grams: Gradient Descent with Adaptive Momentum Scaling. In a similar vein to C-Optim, the parameter update is modified to separate the update direction from momentum. ', 'type': 'bool'},
            'use_adopt': {'title': 'use_adopt', 'tooltip': 'A partial implementation of ADOPT: Modified Adam Can Converge with Any β2 with the Optimal Rate, as we only update the second moment after the parameter update, so as to exclude the current gradient.', 'type': 'bool'},
            'use_focus': {'title': 'use_focus', 'tooltip': 'Modifies the update step to better handle noise at large step sizes. From FOCUS: First-Order Concentrated Update Scheme. This method is incompatible with factorisation (which will increase state memory usage), Muon and Adam-atan2. Additionally, Prodigy modifies the second moment updates when d changes, which may limit the benefits of this method.', 'type': 'bool'},
            'd_limiter': {'title': 'd_limiter', 'tooltip': 'Prevent over-estimated LRs when gradients and EMA are still stabilizing', 'type': 'bool'},
            'use_schedulefree': {'title': 'use_schedulefree', 'tooltip': 'Enable or disable schedulefree functionality. Disabling, will utilize standard Prodigy functionality', 'type': 'bool'},
            'use_orthograd': {'title': 'use_orthograd', 'tooltip': 'Updates weights using the component of the gradient that is orthogonal to the current weight direction, as described in Grokking at the Edge of Numerical Stability. Can help prevent overfitting and improve generalisation.', 'type': 'bool'},
        }
        # @formatter:on
