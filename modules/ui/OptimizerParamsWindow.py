...
class OptimizerParamsWindow(ctk.CTkToplevel):
    ...
    def create_dynamic_ui(
        ...
        KEY_DETAIL_MAP = {
            ...
            'kourkoutas_beta': {'title': 'Kourkoutas Beta', 'tooltip': 'Enables a layer-wise dynamic β₂ adaptation. This feature makes the optimizer more responsive to "spiky" gradients by lowering β₂ during periods of high variance, and more stable during calm periods by raising β₂ towards its maximum. It can significantly improve training stability and final loss on tasks with bursty gradients, such as fine-tuning or training on noisy data.', 'type': 'bool'},
            'k_warmup_steps': {'title': 'K-β Warmup Steps ', 'tooltip': 'When using Kourkoutas Beta, the number of initial training steps during which the dynamic β₂ logic is held off. In this period, β₂ is fixed to an average value to allow for initial training stability before the adaptive mechanism activates.', 'type': 'int'},
            'k_logging': {'title': 'K-β Logging ', 'tooltip': 'If enabled, prints detailed statistics (min, max, and mean) of the dynamic β₂ values across all layers to the console every logging steps. This is useful for debugging.', 'type': 'int'},
        }
        # @formatter:on
