...
class DPOBucketAnalysisWindow(ctk.CTkToplevel):
    def __init__(self, parent, train_config: TrainConfig, *args, **kwargs):
        ...
        self.batch_size_var = ctk.StringVar(
            value=str(max(1, int(getattr(train_config, "batch_size", 1) or 1))))
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(root, text="DPO Bucket / Batch-Size Analyzer",
                     font=("", 22, "bold")).pack(pady=(0, 6))
        ctk.CTkLabel(
            root,
            ...
            font=("", 12), text_color="gray", wraplength=1000, justify="left",
        ).pack(pady=(0, 15))
        ...
        ctk.CTkLabel(folder_card, text="Chosen-side concept folder",
                     font=("", 13, "bold")).pack(anchor="w", padx=15, pady=(12, 4))
        folder_row = ctk.CTkFrame(folder_card, fg_color="transparent")
        folder_row.pack(fill="x", padx=15, pady=(0, 12))
        ctk.CTkButton(folder_row, text="Select Folder", width=150,
                      command=self._select_folder).pack(side="left", padx=(0, 10))
        ...
        ctk.CTkEntry(settings_row, textvariable=self.batch_size_var, width=70).pack(
            side="left", padx=(0, 20))

        ctk.CTkLabel(settings_row, text="Target resolution:").pack(side="left", padx=(0, 6))
        ctk.CTkOptionMenu(settings_row, variable=self.target_var, values=self.target_options,
                          width=90).pack(side="left", padx=(0, 20))

        ctk.CTkLabel(settings_row, text="Quantization:").pack(side="left", padx=(0, 6))
        ctk.CTkOptionMenu(settings_row, variable=self.quant_var, values=self.quant_options,
                          width=80).pack(side="left", padx=(0, 20))

        ctk.CTkButton(settings_row, text="Analyze", width=140,
        ...
        ctk.CTkLabel(root, textvariable=self.status_var, font=("", 12),
    ...
    def _build_empty_results(self):
        ...
        ctk.CTkLabel(self.results_frame,
                     text="Pick a folder and click Analyze to see the bucket breakdown.",
    ...
    def _run_analysis(self):
        ...
        try:
            ...
            messagebox.showwarning("Invalid Batch Size",
                                   "Batch size must be a positive integer.", parent=self)
            return
        try:
        ...
        try:
            ...
            messagebox.showwarning("Invalid Quantization",
                                   "Quantization must be an integer.", parent=self)
            return
        ...
        for widget in self.results_frame.winfo_children():
            widget.destroy()
        ctk.CTkLabel(self.results_frame, text="Scanning images...",
                     font=("", 14)).pack(pady=30)

        self._analysis_thread = threading.Thread(
            target=self._analyze_worker,
            args=(self.concept_path, bs, [target], q), daemon=True)
    ...
    def _render_error(self, message: str):
        ...
        for widget in self.results_frame.winfo_children():
            widget.destroy()
        ctk.CTkLabel(self.results_frame, text=f"Analysis failed: {message}",
                     text_color="red", wraplength=900).pack(pady=20)
    ...
    def _render_result(self, result: dict):
        ...
        summary = (f"Scanned {scanned} image(s)"
                   + (f", {unreadable} unreadable" if unreadable else "")
                   + f"  -  batch_size={bs}, quantization={q}")
        ctk.CTkLabel(self.results_frame, text=summary, font=("", 13),
                     anchor="w").pack(fill="x", padx=5, pady=(0, 10))
    ...
    def _render_target_block(self, target_data: dict, bs: int):
        card = ctk.CTkFrame(self.results_frame, border_width=1,
                            border_color="gray30", corner_radius=8)
        ...
        ctk.CTkLabel(header, text=totals, font=("", 12),
        ...
        for col_idx, (title, weight) in enumerate(zip(columns, weights, strict=True)):
            ...
            ctk.CTkLabel(grid, text=title, font=("", 12, "bold"),
                         anchor="w").grid(row=0, column=col_idx, sticky="we",
                                          padx=6, pady=(0, 4))
        ...
        for col_idx, text in enumerate(footer):
            ctk.CTkLabel(grid, text=text, font=("", 12, "bold"),
                         anchor="w").grid(row=footer_row, column=col_idx,
                                          sticky="we", padx=6, pady=(6, 0))
