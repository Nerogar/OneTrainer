...
class StableDiffusionXLBaseDataLoader(BaseDataLoader):
    def __init__(
            self,
    ...
    def _load_input_modules(self, config: TrainConfig, model: StableDiffusionXLModel) -> list:
        load_image = LoadImage(path_in_name='image_path', image_out_name='image', range_min=0, range_max=1, dtype=model.vae_train_dtype.torch_dtype())

        generate_mask = GenerateImageLike(image_in_name='image', image_out_name='mask', color=255, range_min=0, range_max=1, channels=1)
        load_mask = LoadImage(path_in_name='mask_path', image_out_name='mask', range_min=0, range_max=1, channels=1, dtype=model.vae_train_dtype.torch_dtype())
    ...
    def _cache_modules(self, config: TrainConfig, model: StableDiffusionXLModel):
        ...
        if (not config.text_encoder.train or not config.text_encoder_2.train) and config.latent_caching and not config.train_any_embedding():
