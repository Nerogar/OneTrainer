...
class LoRAModule(metaclass=ABCMeta):
    ...
    def __init__(self, prefix: str, orig_module: nn.Module, rank: int, alpha: float):
        ...
        self.rank = rank
        self.alpha = torch.tensor(alpha).to(orig_module.weight.device)
    ...
    def load_state_dict(self, state_dict: dict):
        down_state_dict = {
            "weight": state_dict[self.prefix + ".lora_down.weight"]
        }
        up_state_dict = {
            "weight": state_dict[self.prefix + ".lora_up.weight"]
        }
        self.alpha = state_dict[self.prefix + ".alpha"]
...
class LoRAModuleWrapper:
    ...
    def __create_modules(self, orig_module: nn.Module, alpha: float) -> dict[str, LoRAModule]:
        lora_modules = {}
