...
@torch.no_grad()
def int8_forward_tokenwise(
    x: Tensor, weight: Tensor, weight_scale: Tensor, bias: Tensor | None, compute_dtype: torch.dtype
) -> Tensor:
...
class LinearInt8Function(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, x: Tensor, weight: Tensor, weight_scale: Tensor, bias: Tensor | None, compute_dtype: torch.dtype
    ) -> Tensor:
...
def benchmark_int8(m, k, n, device="cuda"):
    ...
    run_benchmark(
        lambda: int8_forward_tokenwise(x, w_8, w_scale, bias=None, compute_dtype=torch.bfloat16),
        "torch forward int",
        compile=True,
    )
...
def benchmark_fp8(m, k, n, device="cuda"):
    ...
    run_benchmark(
        lambda: torch._scaled_mm(
            x_8, w_8.T, out_dtype=torch.bfloat16, scale_a=one_scale.float(), scale_b=w_scale.float()
        ),
        "torch mm fp8",
    )
    ...
    def torch_backward(a, b):
        torch._scaled_mm(
            a, b.T.contiguous().T, out_dtype=torch.bfloat16, scale_a=one_scale.float(), scale_b=w_scale.float()
        )
    ...
    run_benchmark(
        lambda: fp8_forward_tokenwise(x, w_8, w_scale, bias=None, compute_dtype=torch.bfloat16),
        "torch forward fp8",
        compile=True,
    )
